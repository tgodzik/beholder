...
trait TableFilterAPI[Entity, Formatter, QueryBase] extends FilterAPI[Entity, Formatter] {
  ...
  override final def filter(data: FilterDefinition)(implicit session: Session): DBIO[Seq[Entity]] =
...
trait FilterAPI[Entity, Formatter] {
  def filter(data: FilterDefinition)(implicit session: Session): DBIO[Seq[Entity]]
